...
func MultiGameListHandler(w http.ResponseWriter, r *http.Request) {
	//DBのマルチゲームでtrueのものを取得する
	query := `
	SELECT t.id, g.code, t.name, t.rule
	FROM types t
	JOIN game_types g ON t.game_type_id = g.id
	WHERE t.mode_id = (SELECT id FROM modes WHERE mode = 'マルチ') AND t.is_can_play = TRUE
	`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &typeCode, &name, &rule); err != nil {
			log.Printf("[ERROR] multi scan failed: %v", err)
